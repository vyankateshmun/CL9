...
public class mpiprogram {
	...
	public static void main(String args[]) {
		...
		int recvbuf[] = new int[2];                                      //Declaring receive buffer
	
		//scatter (it sends chunk of array to different processes)
		
		MPI.COMM_WORLD.Scatter(sendbuf, 0, 2, MPI.INT,    //(Send buffer, Start of the data in buffer, chunk size,datatype)
					 recvbuf, 0, 2, MPI.INT,
					root);

		//double the data

		System.out.println("Process "+rank+" has data: "+recvbuf[0]+" "+recvbuf[1]);		
		recvbuf[0] = (recvbuf[0]+recvbuf[1])/2;
